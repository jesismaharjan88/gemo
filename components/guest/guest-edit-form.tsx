...
// JSX structure mirrors guest-form.tsx intentionally — different submit path (update_response_by_token
// vs insert_response), different prop shape (token + initialValues), different button text.
// Duplication is left in place to avoid touching the working create form. If the shared UI surface
// grows (new fields, redesign) extract a shared MenuPickGrid component and shared field components at
// that point; until then, apply UI changes to both files in tandem.
...
export default function GuestEditForm({
  ...
  const onSubmit = useCallback(
    async (values: GuestFormValues) => {
      ...
      const { error } = await supabase.rpc("update_response_by_token", {
        p_token: token,
      ...
      if (error) {
        if (error.message === "responses_closed") {
          toast.error(
            "This event is no longer accepting changes. Your original response is saved."
          );
        } else {
          toast.error("Couldn't save your changes. Try again or contact the host.");
        }
        return;
      }
