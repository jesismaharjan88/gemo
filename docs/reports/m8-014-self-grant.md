# M8 — 014_rate_limiting.sql: Self-Grant Patch Report

## Step 0 — Identity Args (from the file)

Function identity arguments (read directly from the `CREATE OR REPLACE FUNCTION` header in the file):
...
Full qualified form:
```sql
public.insert_response(uuid, text, text, text, uuid[], text)
```
...
## CREATE OR REPLACE Confirmation

The file already uses `CREATE OR REPLACE FUNCTION` (not `DROP + CREATE`). No conversion was needed. The existing `DROP FUNCTION IF EXISTS` in the file is for the **old 5-parameter** signature only — that DROP removes the legacy backdoor function and is intentionally kept.

---

## Before / After — Grant-Related Lines

**Before:**
```sql
-- Grant: anon-callable (guests are unauthenticated — this matches 010's convention).
REVOKE EXECUTE ON FUNCTION public.insert_response(uuid, text, text, text, uuid[], text) FROM PUBLIC;
...
**What changed:**
- `REVOKE EXECUTE` → `REVOKE ALL` — idempotent: clears any stale/unexpected grants before re-asserting
- `FROM PUBLIC` → `FROM public` — lowercase for consistency
- Comment updated to reflect intent

**Location:** grant block is already at the tail of the file (after the function body), no stray mid-file copies existed. No other grant-related lines were present.

**Function body:** untouched. The `DECLARE` block, rate-limit logic, validation steps, and `RETURN` are identical to the original.

---

## Full Revised File (copy-paste ready)
...
## Verification (run after re-applying in the Supabase SQL editor)
